...
int			buttonscan[NUMBUTTONS] =
			{sc_Control,sc_Alt,sc_RShift,sc_Space,sc_1,sc_2,sc_3,sc_4,sc_None,sc_None,sc_None};
...
void PollKeyboardButtons()
{
	...
	if((state0 & JPF_BUTTON_RED)==JPF_BUTTON_RED
		||(state1 & JPF_BUTTON_RED)==JPF_BUTTON_RED)
		buttonstate[bt_attack] = true;
		
	//Use
	if((state0 & JPF_BUTTON_BLUE)==JPF_BUTTON_BLUE
		||(state1 & JPF_BUTTON_BLUE)==JPF_BUTTON_BLUE)
		buttonstate[bt_use] = true;
		
	//Run
	if((state0 & JPF_BUTTON_GREEN)==JPF_BUTTON_GREEN
		||(state1 & JPF_BUTTON_GREEN)==JPF_BUTTON_GREEN)
		buttonstate[bt_run] = true;
	
	//Toggle gun
	if((state0 & JPF_BUTTON_YELLOW)==JPF_BUTTON_YELLOW
		||(state1 & JPF_BUTTON_YELLOW)==JPF_BUTTON_YELLOW)
		buttonstate[bt_togglegun] = true;
		
	//Strafe Left
	if((state0 & JPF_BUTTON_REVERSE)==JPF_BUTTON_REVERSE
		||(state1 & JPF_BUTTON_REVERSE)==JPF_BUTTON_REVERSE)
		buttonstate[bt_strafeleft] = true;
	
	//Strafe Right
	if((state0 & JPF_BUTTON_FORWARD)==JPF_BUTTON_FORWARD
		||(state1 & JPF_BUTTON_FORWARD)==JPF_BUTTON_FORWARD)
		buttonstate[bt_straferight] = true;	
	
	//Escape
	if((state0 & JPF_BUTTON_PLAY)==JPF_BUTTON_PLAY
		||(state1 & JPF_BUTTON_PLAY)==JPF_BUTTON_PLAY)
		buttonstate[bt_escape] = true;	
...
void PollKeyboardMove()
{
	...
	if (buttonstate[bt_run])
	{
		if (IN_KeyDown(dirscan[di_north]) 
			|| (state0 & JPF_JOY_UP)==JPF_JOY_UP 
			|| (state1 & JPF_JOY_UP)==JPF_JOY_UP)
			controly -= RUNMOVE*tics;
		if (IN_KeyDown(dirscan[di_south]) 
			|| (state0 & JPF_JOY_DOWN)==JPF_JOY_DOWN 
			|| (state1 & JPF_JOY_DOWN)==JPF_JOY_DOWN)
			controly += RUNMOVE*tics;
		if (IN_KeyDown(dirscan[di_west]) 
			|| (state0 & JPF_JOY_LEFT)==JPF_JOY_LEFT 
			|| (state1 & JPF_JOY_LEFT)==JPF_JOY_LEFT)
			controlx -= RUNMOVE*tics;
		if (IN_KeyDown(dirscan[di_east]) 
			|| (state0 & JPF_JOY_RIGHT)==JPF_JOY_RIGHT
			|| (state1 & JPF_JOY_RIGHT)==JPF_JOY_RIGHT)
			controlx += RUNMOVE*tics;
		if ((state0 & JPF_BUTTON_FORWARD)==JPF_BUTTON_FORWARD
			|| (state1 & JPF_BUTTON_FORWARD)==JPF_BUTTON_FORWARD)
			strafe += RUNMOVE*tics;
...
//
	...
	//CD32 play button
	if (buttonstate[bt_escape]) scan=sc_Escape;
