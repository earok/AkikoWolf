...
void DrawWeapon();
void GiveWeapon(byte weapon);
void GiveAmmo(unsigned char ammo);
...
void CheckWeaponChange()
{
	...
	//CD32 toggle through guns
	if(buttonstate[bt_togglegun])
	{
		i = (gamestate.weapon+1);
		if(i>gamestate.bestweapon) i=wp_knife;
...
//
	...
	//
	//
		if (buttonstate[bt_strafeleft] || buttonstate[bt_straferight]) strafeamount = strafe;
		else strafeamount = controlx;
		...
		if (strafeamount > 0)
		{
			...
			if (angle < 0)
				angle += ANGLES;
			Thrust (angle,strafeamount*MOVESCALE);	// move to left
		}
		else if (strafeamount < 0)
		{
			...
			if (angle >= ANGLES)
				angle -= ANGLES;
			Thrust (angle,-strafeamount*MOVESCALE);	// move to right
		}
	}
...
==================
*/
unsigned char oldface=0;
unsigned char oldfaceframe=0;
signed char rf=-2;
void DrawFace()
{

	if (gamestate.health) {
	if (oldface!=gamestate.health || oldfaceframe!=gamestate.faceframe || drawScr==1)
	{
		#ifdef SPEAR
		if (godmode)
		...
		#endif
		StatusDrawPic (17,4,FACE1APIC+3*((100-gamestate.health)>>4)+gamestate.faceframe);
rf=-2;
oldfaceframe=gamestate.faceframe;
oldface=gamestate.health;
}
	} else {
#ifndef SPEAR
...
void LatchNumber(short x, short y, byte width, long number)
{
	byte length, c;
...
===============
*/
unsigned char oldhealth = 0;
signed char rh = -2;
void DrawHealth()
{
if (oldhealth != gamestate.health || drawScr==1)
{
	LatchNumber(21,16,3,gamestate.health);
	rh = -2;
	}
	oldhealth=gamestate.health;
}
...
void DrawLevel()
{
if (oldlevel != gamestate.mapon+1 || drawScr==1)
{
#ifdef SPEAR
...
===============
*/
unsigned char oldlives=0;
signed char rl = -2;
void DrawLives()
{
if (oldlives!=gamestate.lives || drawScr==1)
{
	LatchNumber(14,16,1,gamestate.lives);
	rl = -2;
	}
oldlives=gamestate.lives;	
}
...
unsigned short oldscore=1;
signed char rs = -2;
void DrawScore()
{
if (oldscore != gamestate.score || drawScr==1)
{
...
*/
signed char rw=-2;
unsigned char oldweapon=0;
void DrawWeapon()
{
if (oldweapon!=gamestate.weapon || drawScr==1)
{
...
*/
signed char rk=-2;
unsigned char oldkeys = 1;
void DrawKeys()
{
if (gamestate.keys != oldkeys || drawScr==1)
{
	if (gamestate.keys & 1)
...
*/
 signed char ra=-2;
unsigned char oldammo = 0;
void DrawAmmo()
{
	if (oldammo!=gamestate.ammo || drawScr==1)
	{
	LatchNumber(27,16,2,gamestate.ammo);
			ra=-2;
		}
	oldammo = gamestate.ammo;
}
