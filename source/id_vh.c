...
static void VL_Plot(int x, int y, byte color)
{
	int xend, yend;
    short xs, ys;
//	  if (xfrac!=1 && yfrac!=1)
   // {
	...
	for (xs = (short)x; xs < (short)xend; xs++)
		for (ys = (short)y; ys < (short)yend; ys++)
			*(gfxbuf + ys * 320 + xs) = color;
            //}
          //  else
          //  {
          	//		    *(gfxbuf + (y>>16) * vstride + (x>>16)) = color;
             //   }
}

void VW_Plot(int x, int y, byte color)
{
    	int xend, yend;
    short xs, ys;
//	  if (xfrac!=1 && yfrac!=1)
...
{
	byte *font;
	short width, step, height, x, xs, y;
	byte *source, *ptrs;
	byte ch;
	...
	while ((ch = *string++) != 0) {
		...
		source = font+font[2+ch+ch+0]+(font[2+ch+ch+1]<<8);
		for (x = 0; x < width; x++) {
			ptrs = source;
			for (y = 0; y < height; y++) {
				if (*ptrs)
...
{
	VL_MemToScreen(grsegs[STARTTILE8]+(tile<<6), 8, 8, x, y);
}
...
void VWB_DrawPic(int x, int y, int chunknum)
{
	...
	width = (short)pictable[picnum].width;
	height = (short)pictable[picnum].height;
...
*/
	...
	short w;
///	   unsigned long *dst;
 //   dst=(unsigned long*)(gfxbuf+y*vstride+x);
	for (w = 0; w < width; w++)
//   {
//    *dst=(color)|(color<<8)|(color<<16)|(color<<24);
 //   dst++;
 //   }
   	  VL_Plot(x+w, y, color);
...
void VL_Vlin(int x, int y, int height, byte color)
{
/*
	int xend, yend;
	int w, h;
...
void VW_Bar(int x, int y, int width, int height, byte color)
{
	int w, h;
	byte *ptr;
...
void VL_Bar(int x, int y, int width, int height, byte color)
{
...
void VL_MemToScreen(const byte *source, int width, int height, int x, int y)
{
   ...
   // dst=(unsigned long*)gfxbuf+320*(y)+x;
    src=(unsigned long*)source;
     for (h = 0; h < (byte)height; h++)
 
    {    dst=(unsigned long*)(gfxbuf+320*(y+h)+x);
         for (w = 0; w < (short)(width >> 2); w++)
        {
            *dst=*src;
            dst++;
            src++;
