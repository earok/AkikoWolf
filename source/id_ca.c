...
static FILE * grhandle = 0;   /* handle to VGAGRAPH */
static FILE * maphandle = 0;  /* handle to GAMEMAPS */
static FILE * audiohandle = 0;    /* handle to AUDIOT */
...
boolean CA_WriteFile(char *filename, void *ptr, long length)
{
	...
	if (l == -1) {
		...
		//printf(stderr, "CA_FarWrite hit EOF?\n");
		return false;
	} else if (l != length) {
		//printf(stderr, "CA_FarWrite only wrote %d out of %ld\n", l, length);
		return false;
	}
...
boolean CA_LoadFile(char *filename, memptr *ptr)
{
	...
	if (l == -1) {
		...
		//printf(stderr, "CA_FarRead hit EOF?\n");
		return false;
	} else if (l != size) {
		//printf(stderr, "CA_FarRead only read %d out of %ld\n", l, size);
		return false;
	}
...
void CA_UnCacheAudioChunk(int chunk)
{
	if (audiosegs[chunk] == 0) {
		//printf(stderr, "Trying to free null audio chunk %d!\n", chunk);
		return;
	}
...
{
	if (grsegs[chunk] == 0) {
		//printf(stderr, "Trying to free null pointer %d!\n", chunk);
		return;
	}
