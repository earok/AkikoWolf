...
static short int sdlbuf[2048];
//static short int sdldbuf[2048];
...
#define alWave		0xE0
			...
			for (i = 0; i < (sizeof(sdlbuf)/sizeof(sdlbuf[0])); i += 2) {
				if (SoundPlaying != -1) {
					if (SoundPositioned) {
						samp = (SoundData[(SoundPlayPos >> 16)] << 8)^0x8000;
						snd = samp*(16-L)/32+musbuf[i/8];
...
//						}
						...
						snd = (((signed short)((SoundData[(SoundPlayPos >> 16)] << 8)^0x8000))>>2)+musbuf[i/8];
						if (snd > 32767)
							snd = 32767;
						if (snd < -32768)
							snd = -32768;
...
//						}
					...
					SoundPlayPos += 20805; // 7000 / 44100 * 65536
...
//					if (usebuf) {
					  sdlbuf[i] = musbuf[i/8];
					  sdlbuf[i+1] = musbuf[i/8];
//					} else {
//					  sdldbuf[i] = musbuf[i/8];
//					  sdldbuf[i+1] = musbuf[i/8];
//					}
				}
			}
...
//printf("%i - %i - %i\n",bufpos,sizeof(sdlbuf),len);

//  if (usebuf) {
//    waveptr = (char*)sdldbuf + bufpos;
//  } else {
    waveptr = (char*)sdlbuf + bufpos;
//  }
    waveleft = sizeof(sdlbuf) - bufpos;

    while ( waveleft <= len ) {
   //   SDL_MixAudio(stream, waveptr, waveleft, Volume*SDL_MIX_MAXVOLUME/10);
      stream += waveleft;
      len -= waveleft;
//      if (usebuf) {
//        waveptr = (char*)sdldbuf;
//      } else {
        waveptr = (char*)sdlbuf;
//      }
      waveleft = sizeof(sdlbuf);
      bufpos = 0;
    }
...
void Blah()
{
	...
	if (DigiList!=NULL) free(DigiList);
...
//	  SDL_AudioSpec aspec;
	...
	Blah();
	InitDigiMap();
	OPL = OPLCreate(OPL_TYPE_YM3812, 14318176, 22050);
	audiofd=0;
...
/*	  aspec.freq = 22050;
	...
	aspec.channels = 2; //PK 2;
	aspec.samples = 1024;//sizeof(sdlbuf)/4;
