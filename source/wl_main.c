...
========================
*/
/*fixed __asm_fixed(__reg("d2") fixed, __reg("d0") fixed )=
                "\tmuls.l\td2,d1:d0\n"
                "\tmove.w\td1,d0\n"
                "\tswap\td0\n"
#define xFixedByFrac(x,y) __asm_fixed(x,y)*/
...
int ReadSaveTag2(char *fn, char *tag)
{
    FILE *file;
    char buf[8];
    int32_t v;
...
void NewViewSize(int width)
{
	...
	if ((width*16*HEIGHTRATIO) > (vheight - 40*vheight/HEIGHT))
		width = (vheight - 40*vheight/HEIGHT)/8;
	
	viewwidthwin = width*16*WIDTH/vwidth;
	viewheightwin = width*16*HEIGHTRATIO*WIDTH/vwidth;
	viewsize = width*WIDTH/vwidth;
	
	viewwidth = width<<4;
	viewheight = width*16*HEIGHTRATIO;
	
	centerx = (viewwidth>>1)-1;
	shootdelta = viewwidth/10;
	
	yoffset = (vheight-STATUSLINES*vheight/HEIGHT-viewheight)>>1;
	xoffset = (vwidth-viewwidth)>>1;
	
	yoffset320 = 320 * yoffset;
	step = (viewheight>>1)*80;
//
